...
if ($row['radminsuper'] == 1) {
	...
	function moderation_news_view($id) {
		...
		if ($comm['pid'] != 0) {
			$reply = $db->sql_fetchrow($db->sql_query("SELECT name, subject, comment FROM ".$prefix."_comments WHERE tid=".intval($comm['pid'])));
		} else {
			$reply = "";	
		}
