...
/************************************************************************/
/* $Id: index.php,v 1.1.1.2 2005/11/21 13:51:49 tropic Exp $ */
...
#== // Php-MultiShop
		$result = $db->sql_query("INSERT INTO ".$user_prefix."_users_temp (user_id, username, user_email, user_password, customers_password, user_regdate, check_num, time) VALUES (NULL, '$username', '$user_email', '$new_password', '$crypted_password', '$user_regdate', '$check_num', '$time')");
#== //
		if(!$result) {
			echo ""._ERROR."<br>";
		} else {
...
#== // Php-MultiShop
		    ...
		    $db->sql_query("INSERT INTO ".$user_prefix."_users (user_id, name, username, user_email, user_password, user_avatar, user_avatar_type, user_regdate, user_lang) VALUES ('".$new_uid."', '".$row['username']."', '".$row['username']."', '".$row['user_email']."', '$user_password', 'gallery/blank.gif', '3', '".$row['user_regdate']."', '$language')");
#== //
...
#== // Php-MultiShop
		    $db->sql_query("INSERT INTO " . TABLE_ADDRESS_BOOK . " (customers_id, entry_firstname) VALUES ('$new_uid','$row[username]') ");
		    $address_id = $db->sql_nextid();
		    $db->sql_query("INSERT INTO customers (customers_id, customers_firstname, customers_email_address, customers_password, customers_default_address_id) VALUES ($new_uid, '$row[username]', '$row[user_email]', '$row[customers_password]', '$address_id')");
#== //
...
#== // Php-MultiShop
	$sql = "SELECT user_id, user_email, user_password FROM ".$user_prefix."_users WHERE username='$username'";
#== //
...
#== // Php-MultiShop
		    	...
		    	$query2 = "UPDATE customers SET customers_password='$crypted_password' WHERE customers_id='".$row['user_id']."'";	    	
		    	$db->sql_query($query2);
#== //
			}
			include ("header.php");
...
function editshop() {
    global $user, $db, $userinfo, $cookie, $module_name, $bgcolor1, $bgcolor2, $bgcolor3;
    cookiedecode($user);
    getusrinfo($user);
	if ((is_user($user)) AND (strtolower($userinfo['username']) == strtolower($cookie[1])) AND ($userinfo['user_password'] == $cookie[2])) {
    
		$account_query = $db->sql_query("select customers_gender, customers_firstname, customers_lastname, customers_default_address_id, customers_dob, customers_email_address, entry_company, entry_street_address, entry_suburb, entry_postcode, entry_city, entry_zone_id, entry_state, entry_country_id, customers_telephone, customers_fax, customers_newsletter from " . TABLE_CUSTOMERS . " left join  " . TABLE_ADDRESS_BOOK . " on customers.customers_default_address_id = address_book.address_book_id where customers.customers_id = '" . $userinfo['user_id'] . "' and address_book.customers_id = '" . $userinfo['user_id'] . "' ");
		$personalinfo = $db->sql_fetchrow($account_query);
		$address_id = $personalinfo['customers_default_address_id'];

		include("header.php");
        OpenTable();
        echo "<center><font class=\"title\"><b>"._INFORMATION_FOR_SHOPPING."</b></font></center>\n\n";
        CloseTable();
        echo "<br>";
        ...
        echo "<br>\n\n";
        OpenTable();
        echo "<table cellpadding=\"3\" border=\"0\" width='100%'><tr><td bgcolor='$bgcolor1'>\n"
            ."<form name=\"Register\" action=\"modules.php?name=$module_name\" method=\"post\">\n"
            ."<b>"._USRNICKNAME."</b>:</td><td bgcolor='$bgcolor3'><b>" . htmlentities($userinfo[username]) . "</b></td></tr>\n";

		echo "<tr><td bgcolor='$bgcolor1'><b>"._UREALEMAIL.":</b><br>"._REQUIRED."</td>\n"
        ...
        echo "<tr><td bgcolor='$bgcolor1'><b>"._PASSWORD."</b>:</td><br>\n"
            ."<td bgcolor='$bgcolor3'>"
            ."<input type=\"password\" name=\"user_password\" size=\"22\" maxlength=\"20\">&nbsp;&nbsp;"
            ."<input type=\"password\" name=\"vpass\" size=\"22\" maxlength=\"20\"><br>"._TYPENEWPASSWORD."</td></tr>\n";

#== // Php-MultiShop
		echo "<tr><td bgcolor='$bgcolor1'><b>". _FIRST_NAME . ":</td>"
            ."<td bgcolor='$bgcolor3'>";
  		if ($is_read_only) {
    		echo $personalinfo['customers_firstname'];
  		} elseif ($error) {
    		...
    		echo tep_draw_input_field('firstname', $personalinfo['customers_firstname']) . ' ' . ENTRY_FIRST_NAME_TEXT;
  		}
		echo "</td></tr>\n";
		echo "<tr><td bgcolor='$bgcolor1'><b>" ._LAST_NAME .":</td>"
			."<td bgcolor='$bgcolor3'>";
  		if ($is_read_only) {
    		echo $personalinfo['customers_lastname'];
  		} elseif ($error) {
    		...
    		echo tep_draw_input_field('lastname', $personalinfo['customers_lastname']) . ' ' . ENTRY_LAST_NAME_TEXT;
  		}
		echo "</td></tr>\n";

  		if (ACCOUNT_COMPANY == 'true') {
			echo "<tr><td bgcolor='$bgcolor1'><b>" . _COMPANY .":</td>"
            	    ."<td bgcolor='$bgcolor3'>";
    		if ($is_read_only) {
      			echo $personalinfo['entry_company'];
    		} elseif ($error) {
      			...
      			echo tep_draw_input_field('company', $personalinfo['entry_company']) . ' ' . ENTRY_COMPANY_TEXT;
    		}
			echo "</td></tr>\n";
  		}
		echo "<tr><td bgcolor='$bgcolor1' VALIGN=TOP><b>". _STREET_ADDRESS . ":</td>"
			."<td bgcolor='$bgcolor3'>";
  		if ($is_read_only) {
    		echo $personalinfo['entry_street_address'];
  		} elseif ($error) {
    		...
    		echo tep_draw_input_field('street_address', $personalinfo['entry_street_address']) . ' ' . ENTRY_STREET_ADDRESS_TEXT;
  		}
  
  		if (ACCOUNT_SUBURB == 'true') {
			echo "<BR>";
    		if ($is_read_only) {
      			echo $personalinfo['entry_suburb'];
    		} elseif ($error) {
      			...
      			echo tep_draw_input_field('suburb', $personalinfo['entry_suburb']) . ' ' . ENTRY_SUBURB_TEXT;
      			echo " [". _SUBURB . "]";
    		}
  		}
		echo "</td></tr>\n";
		echo "<tr><td bgcolor='$bgcolor1'><b>" . _POST_CODE .":</TD>"
			."<td bgcolor='$bgcolor3'>";
  		if ($is_read_only) {
    		echo $personalinfo['entry_postcode'];
  		} elseif ($error) {
    		...
    		echo tep_draw_input_field('postcode', $personalinfo['entry_postcode']) . ' ' . ENTRY_POST_CODE_TEXT;
  		}
		echo "</td></tr>";
		echo "<tr><td bgcolor='$bgcolor1'><b>". _CITY .":</td>"
			."<td bgcolor='$bgcolor3'>";
  		if ($is_read_only) {
    		echo $personalinfo['entry_city'];
  		} elseif ($error) {
    		if ($entry_city_error) {
    		...
    		echo tep_draw_input_field('city', $personalinfo['entry_city']) . ' ' . ENTRY_CITY_TEXT;
  		}
		echo "</td></tr>\n";
		echo "<tr><td bgcolor='$bgcolor1'><b>". _STATE_PROVINCE. ":</TD>"
            	    ."<td bgcolor='$bgcolor3'>";
    	$state = tep_get_zone_name($country, $zone_id, $state);
    	if ($is_read_only) {
      		echo tep_get_zone_name($personalinfo['entry_country_id'], $personalinfo['entry_zone_id'], $personalinfo['entry_state']);
    	} elseif ($error) {
      		if ($entry_state_error) {
        		if ($entry_state_has_zones) {
          			$zones_array = array();
          			$zones_query = $db->sql_query("select zone_name from " . TABLE_ZONES . " where zone_country_id = '" . tep_db_input($country) . "' order by zone_name");
          			while ($zones_values = $db->sql_fetchrow($zones_query)) {
      		...
      		echo tep_draw_input_field('state', tep_get_zone_name($personalinfo['entry_country_id'], $personalinfo['entry_zone_id'], $personalinfo['entry_state'])) . ' ' . ENTRY_STATE_TEXT;
    	}
		echo "</td></tr>";
		echo "<tr><td bgcolor='$bgcolor1'><b>" . _COUNTRY .":</TD>"
			."<td bgcolor='$bgcolor3'>";
  		if ($is_read_only) {
    		echo tep_get_country_name($personalinfo['entry_country_id']);
  		} elseif ($error) {
    		...
    		echo tep_get_country_list('country', $personalinfo['entry_country_id']) . ' ' . ENTRY_COUNTRY_TEXT;
  		}
		echo "</td></tr>";
		echo "<tr><td bgcolor='$bgcolor1'><b>" . _TELEPHONE .":</TD>"
			."<td bgcolor='$bgcolor3'>";
  		if ($is_read_only) {
    		echo $personalinfo['customers_telephone'];
  		} elseif ($error) {
    		...
    		echo tep_draw_input_field('telephone', $personalinfo['customers_telephone']) . ' ' . ENTRY_TELEPHONE_NUMBER_TEXT;
  		}
		echo "</td></tr>";
		echo "<tr><td bgcolor='$bgcolor1'><b>Fax".":</td>"
			."<td bgcolor='$bgcolor3'>";
  		if ($is_read_only) {
    		echo $personalinfo['customers_fax'];
  		} elseif ($processed) {
    		echo $fax . tep_draw_hidden_field('fax');
  		} else {
    		echo tep_draw_input_field('fax', $personalinfo['customers_fax']) . ' ' . ENTRY_FAX_NUMBER_TEXT;
  		}
		echo "</td></tr>";

  		if (ACCOUNT_GENDER == 'true') {
      		$male = ($personalinfo['customers_gender'] == 'm') ? true : false;
    		$female = ($personalinfo['customers_gender'] == 'f') ? true : false;

			echo "<tr><td bgcolor='$bgcolor1'><b>" . _GENDER .":</TD><td bgcolor='$bgcolor3'>";
    		if ($is_read_only) {
      			echo ($personalinfo['customers_gender'] == 'm') ? MALE : FEMALE;
    		} elseif ($error) {
  		...
  		if (ACCOUNT_DOB == 'true') {
			echo "<tr><td bgcolor='$bgcolor1'><b>" . _DATE_OF_BIRTH .":</TD>"
				."<td bgcolor='$bgcolor3'>";
    		if ($is_read_only) {
      			echo tep_date_short($personalinfo['customers_dob']);
    		} elseif ($error) {
      		...
      		echo tep_draw_input_field('dob', tep_date_short($personalinfo['customers_dob'])) . ' ' . ENTRY_DATE_OF_BIRTH_TEXT;
    		}
			echo "</td></tr>";
  		}
#== //
        ...
        echo "<tr><td bgcolor='$bgcolor3' colspan='2' align='center'>"
            ...
            ."<input type=\"hidden\" name=\"default_address_id\" value=\"$address_id]\">"
            ."<input type=\"hidden\" name=\"op\" value=\"saveshop\">"
...
function saveshop($user_email, $user_password, $vpass, $username, $user_id, $firstname, $lastname, $default_address_id, $gender, $dob, $telephone, $fax, $company, $street_address, $suburb, $postcode, $city, $zone_id, $state, $country) {
...
#== // Php-MultiShop
	  	...
	  	$telephone = tep_db_prepare_input($telephone);
	  	$fax = tep_db_prepare_input($fax);
	  	$gender = tep_db_prepare_input($gender);
	  	$dob = tep_db_prepare_input($dob);
	  	$company = tep_db_prepare_input($company);
	  	$street_address = tep_db_prepare_input($street_address);
	  	...
	  	if ((!$user_email) || ($user_email=="") || (!eregi("^[_\.0-9a-z-]+@([0-9a-z][0-9a-z-]+\.)+[a-z]{2,6}$",$user_email))) {
	    	$error = true;
	    	$report_error .= "<li>" . _EMAIL . "</li>";
	  	}
	
	  	if (strlen($firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
	    	$error = true;
	    	$report_error .= "<li>" . _FIRST_NAME . "</li>";
	  	...
	  	if (strlen($street_address) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
	    	$error = true;
	    	$report_error .= "<li>" . _STREET_ADDRESS . "</li>";
	    	$entry_street_address_error = true;
	  	} else {
	    	$entry_street_address_error = false;
	  	}
	
	  	if (!is_numeric($postcode) || strlen($postcode) < ENTRY_POSTCODE_MIN_LENGTH) {
	    	$error = true;
	    	$report_error .= "<li>" . _POST_CODE . "</li>";
	  	...
	  	if (strlen($city) < ENTRY_CITY_MIN_LENGTH) {
	    	$error = true;
	    	$report_error .= "<li>" . _CITY . "</li>";
	    	$entry_city_error = true;
	  	} else {
	    	$entry_city_error = false;
	  	}
	  
	  	if (strlen($state) < ENTRY_STATE_PROVINCE_MIN_LENGTH) {
	    	$error = true;
	    	$report_error .= "<li>" . _STATE_PROVINCE . "</li>";
	    	$entry_city_error = true;
	  	} else {
	    	$entry_city_error = false;
	  	}
	  	...
	  	if (ACCOUNT_STATE == 'true') {
	    	if ($entry_country_error) {
	      		...
	      		$country_check_query = $db->sql_query("select count(*) as total from " . TABLE_ZONES . " where zone_country_id = '" . tep_db_input($country) . "'");
	      		$country_check = $db->sql_fetchrow($country_check_query);
	      		if ($entry_state_has_zones = ($country_check['total'] > 0)) {
	        		$match_zone_query = $db->sql_query("select zone_id from " . TABLE_ZONES . " where zone_country_id = '" . tep_db_input($country) . "' and zone_name = '" . tep_db_input($state) . "'");
	        		if ($db->sql_numrows($match_zone_query) == 1) {
	          			$match_zone = $db->sql_fetchrow($match_zone_query);
	          			$zone_id = $match_zone['zone_id'];
	        		} else {
	          			$match_zone_query = $db->sql_query("select zone_id from " . TABLE_ZONES . " where zone_country_id = '" . tep_db_input($country) . "' and zone_code = '" . tep_db_input($state) . "'");
	          			if ($db->sql_numrows($match_zone_query) == 1) {
	            			$match_zone = $db->sql_fetchrow($match_zone_query);
	            			$zone_id = $match_zone['zone_id'];
	          			} else {
	  	...
	  	if (!is_numeric($telephone) || strlen($telephone) < ENTRY_TELEPHONE_MIN_LENGTH) {
	    	$error = true;
	    	$report_error .= "<li>" . _TELEPHONE . "</li>";
	  	...
	  	if (ACCOUNT_DOB == 'true') {
	    	if (checkdate(substr(tep_date_raw($dob), 4, 2), substr(tep_date_raw($dob), 6, 2), substr(tep_date_raw($dob), 0, 4))) {
	      		...
	      		$error = true;
	      		$report_error .= "<li>" . _DATE_OF_BIRTH . "</li>";
		...
		if ((isset($user_password)) && ("$user_password" != "$vpass")) {
			...
			echo "<br>";
			echo "<br><font class=\"title\">ERROR: </font>" ._REQUIRED_INFORMATION;
			echo "<center><table><tr><td>$report_error</td></tr></table></center>";
			CloseTable();
			include("footer.php");
		} else { 
			// make changes
			if ($user_password != "") {
	            cookiedecode($user);
	            $db->sql_query("LOCK TABLES ".$user_prefix."_users, customers WRITE");
	            $nuke_crypt_password = md5($user_password);

#== // update password in nuke_users table
	            $db->sql_query("UPDATE ".$user_prefix."_users SET user_email='$user_email', user_password='$nuke_crypt_password' WHERE user_id='$user_id'");

#== // update password in customers table
	            $osc_crypt_password = tep_encrypt_password($user_password);
	            $db->sql_query("UPDATE customers SET customers_email_address ='$user_email', customers_password='$osc_crypt_password' WHERE customers_id='$user_id'");
								
	            $result = $db->sql_query("SELECT user_id, username, user_password, storynum, umode, uorder, thold, noscore, ublockon, theme FROM ".$user_prefix."_users WHERE user_id='$user_id' AND user_password='$nuke_crypt_password'");
	            if ($db->sql_numrows($result) == 1) {
	            ...
	            $db->sql_query("UPDATE ".$user_prefix."_users SET user_email='$user_email' WHERE user_id='$user_id'");

	            $db->sql_query("UPDATE customers SET customers_email_address ='$user_email' WHERE customers_id='$user_id'");
	        }
...
#=================
	    	...
	    	tep_db_perform(TABLE_CUSTOMERS, $sql_data_array, 'update', "customers_id = '" . $user_id . "'");
		    ...
		    tep_db_perform(TABLE_ADDRESS_BOOK, $sql_data_array, 'update', "customers_id = '" . $user_id . "' and address_book_id = '$default_address_id'");
...
#== // Php-MultiShop
				$osc_crypt_passw = tep_encrypt_password($user_password);
				$db->sql_query("UPDATE customers SET customers_password ='$osc_crypt_passw' WHERE customers_id='$user_id'");
#== //			
...
switch($op) {
    ...
    case "saveshop":
	saveshop($user_email, $user_password, $vpass, $username, $user_id, $firstname, $lastname, $default_address_id, $gender, $dob, $telephone, $fax, $company, $street_address, $suburb, $postcode, $city, $zone_id, $state, $country);
    break;
	...
	break;
        ...
        case "my_headlines": // SecurityReason.com Fix 2005 [sp3x]
	  if (is_user($user)) {
		if (!empty($url)) {
