...
#== // MultiShop
if (file_exists(DIR_WS_INCLUDES . "application_top.php")){
	require_once(DIR_WS_INCLUDES . "application_top.php");
} else {
	die("<b>Warning</b>: check your configuration in \"<i>config.php</i>\" file.<br><br>"
		."The defined constant '<i>DIR_FS_DOCUMENT_ROOT</i>' "
		."should be someting like:<br> " . dirname($_SERVER["SCRIPT_FILENAME"]) . "/") ;
}
#== //
...
$Multishop_Version = $Conf_Multishop['Multishop_Version'];
$multishop_copyright = $Conf_Multishop['multishop_copyright'];
$conf_query = $db->sql_query('select configuration_key as cfgKey, configuration_value as cfgValue from configuration_common_stores');
	while ($conf = $db->sql_fetchrow($conf_query)) {
...
if (isset($newcurrency)){
	setcookie("currency",$newcurrency,time()+31536000);
	$currentcurrency = $newcurrency;
	$currency = $currentcurrency;
	
} elseif ($_SESSION['currency'] && $_SESSION['currency'] != $_COOKIE['currency']){
	$currentcurrency = $_SESSION['currency'];
	setcookie("currency",$currentcurrency,time()+31536000);
	$currency = $currentcurrency;
	
} elseif ($_COOKIE['currency'] && !$_SESSION['currency']){
...
#== // MultiShop
      if (isset($uname)) {
      	$db->sql_query("UPDATE ".$prefix."_session SET random_num = '$Random_num' WHERE uname='$uname' and guest='0'");
	  }
#== //
    } else {
