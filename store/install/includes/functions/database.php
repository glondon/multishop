<?php
/*
  $Id: database.php,v 1.2 2004/05/30 21:51:14 tropic Exp $
    ...
    if (!$db_error) {
      if (!@osc_db_select_db($database)) {
        $db_error = mysql_error();
      	...
      	// change for Php-MultiShop
      	$table_configuration = $prefix . "_configuration";
        if (!@osc_db_query('select count(*) from '.$table_configuration)) {
          $db_error = mysql_error();
        }
      }
    }
