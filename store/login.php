<?php
/*
  $Id: login.php,v 1.3 2004/05/31 14:04:34 tropic Exp $
...
/** check usession to login
 **/

if(($check_login = check_session()) && $usession){
	$customers_id = get_userid();
	$check_customer_query = tep_db_query("select customers_id, customers_firstname, customers_password, customers_email_address, customers_default_address_id from " . TABLE_CUSTOMERS . " where customers_id = '" . tep_db_input($customers_id) . "'");
...
// restore cart contents
        ...
        if (!($redirect = $HTTP_GET_VARS['redirect'])){
        	tep_redirect(tep_href_link(FILENAME_DEFAULT));
        } else {
        	if ($_GET) {
  			foreach ($_GET as $k => $v) {
    				$_GET[$k] = $v;
    				if (($k != "usession") && ($k != "redirect")){
    					$MyGet .= "$k=$v&";
    				}
  			}
		}
		$redirect .= "?$MyGet&" . tep_session_name() . "=". tep_session_id();
        }
        header('Location: ' . $redirect);
        tep_exit();
    }
} elseif ($redirect = $HTTP_GET_VARS['redirect']){
