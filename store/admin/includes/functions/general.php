<?php
/*
  $Id: general.php,v 1.4 2005/11/23 00:29:06 tropic Exp $
...
/** Php-MultiShop
 ** function to check in table 'privacy' 
 ** if the customer is readable for this Shop
 **/
function customer_readable($customers_id,$vendors_id){
	
	$return = 0 ;	
	if (mysql_num_rows ( mysql_query("SELECT customers_id FROM privacy WHERE customers_id='$customers_id' and allow_vendors_id = '$vendors_id'")) > 0) $return = 1 ;
	return $return;
}

function this_vendors_id(){
	$query = tep_db_query("select vendors_id from " . PREFIX_DB_NUKE . "_vendors where vendors_prefix = '" . STORE_PREFIX . "'");
	$result = tep_db_fetch_array($query);
	$vendors_id = $result['vendors_id'];
	if ($vendors_id){
		return $vendors_id;
	} else {
		return 0;
