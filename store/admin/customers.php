<?php
/*
  $Id: customers.php,v 1.4 2005/11/23 11:01:25 tropic Exp $
...
#============= Php-MultiShop
	$Alert = "";
	$this_vendors_id = intval(this_vendors_id());
	if($this_vendors_id == 0){
		$Alert ="<br>&nbsp;&nbsp;<b>Note</b>: This Shop is not yet inserted in Your Virtual Mall.";
	}
#=============
...
?>
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <?
          if ($Alert |= ""){
          	echo "<tr><td class=\"main\">$Alert</td></tr>";
          }
          ?>
...
<?php
    ...
    if (isset($HTTP_GET_VARS['search']) && tep_not_null($HTTP_GET_VARS['search'])) {
      ...
      $search = "where p.allow_vendors_id = $this_vendors_id and (c.customers_lastname like '%" . $keywords . "%' or c.customers_firstname like '%" . $keywords . "%' or c.customers_email_address like '%" . $keywords . "%')";
    } else {
#============= Php-MultiShop
      $search = "where p.allow_vendors_id = $this_vendors_id";
#=============
    }
...
//        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=confirm') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a> <a href="' . tep_href_link(FILENAME_ORDERS, 'cID=' . $cInfo->customers_id) . '">' . tep_image_button('button_orders.gif', IMAGE_ORDERS) . '</a> <a href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $cInfo->customers_email_address) . '">' . tep_image_button('button_email.gif', IMAGE_EMAIL) . '</a>');
        $contents[] = array('align' => 'center', 'text' => '</a> <a href="' . tep_href_link(FILENAME_ORDERS, 'cID=' . $cInfo->customers_id) . '">' . tep_image_button('button_orders.gif', IMAGE_ORDERS) . '</a> <a href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $cInfo->customers_email_address) . '">' . tep_image_button('button_email.gif', IMAGE_EMAIL) . '</a>');
